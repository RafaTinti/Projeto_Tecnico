<x-app-layout>
    ...
    <div class=" w-2/3  mx-auto p-4 sm:p-6 lg:p-8">
        <form action="{{ route("Categorias.update", $categoria->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="categoria" value="{{$categoria->categoria}}" class="shadow appearance-none border rounded w-full py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <div class="flex  my-1 justify-between">
                    ...
                    <select name="tipo" id="tipo" class=" shadow appearance-none border rounded w-1/2 py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <option value="debito" {{($categoria->tipo==="debito")? "selected" :"" }} >Débito</option>
                        <option value="credito" {{($categoria->tipo==="credito")? "selected" :"" }}>Crédito</option>
                    </select>
            </div>
