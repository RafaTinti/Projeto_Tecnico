<x-app-layout>
    ...
    <div class=" w-2/3  mx-auto p-4 sm:p-6 lg:p-8">
        <form action="{{ route("Transacao.update", $transacao->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')

            <input type="text" name="descricao" value="{{ $transacao->descricao }}" class="shadow appearance-none border rounded w-full py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <div class="flex my-1 justify-between">
                ...
                <select name="tipo" id="tipo" class=" shadow appearance-none border rounded w-1/2 py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="debito" {{ $transacao->categoria->tipo === "debito"? "selected" : ""}}>Débito</option>
                    <option value="credito" {{ $transacao->categoria->tipo === "credito"? "selected" : ""}}>Crédito</option>
                </select>
            </div>
                ...
                <select name="categoria_id" id="categoriaDebito" class=" shadow appearance-none border rounded w-1/2 py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    @foreach ($categorias as $categoria)
                        @if ($categoria->tipo === "debito")
                            <option value="{{ $categoria->id }}" {{ ($categoria->id === $transacao->categoria->id) ? "selected" : "" }}>{{$categoria->categoria}}</option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="flex my-1 justify-between">
                ...
                <select name="status" id="status" class=" shadow appearance-none border rounded w-1/2 py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="pendente" {{ $categoria->status === "pendente"? "selected" : "" }}>Pendente</option>
                    <option value="liquidada" {{ $categoria->status === "liquidada"? "selected" : "" }}>Liquidada</option>
                </select>
            </div>

            <input type="number" name="valor" min="1" step="any" value="{{ $transacao->valor/100 }}" class="shadow appearance-none border rounded w-full py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " />
            ...
            <div class="flex my-1 justify-between">
                ...
                <select name="pessoa_id" id="pessoa_id" class=" shadow appearance-none border rounded w-1/2 py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    <option value="{{$pessoa->id}}" {{ ($pessoa->id === $transacao->pessoa->id) ? "selected" : "" }}>{{ $pessoa->nome }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex my-1 justify-between">
            ...
            <div class="flex my-1 justify-between">
                <input type="date" id="vencimento" name="vencimento" value="{{ \Carbon\Carbon::parse($transacao->vencimento)->format("Y-m-d")}}" class="shadow appearance-none border rounded w-full py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline "/>
                <input type="date" id="liquidada" name="liquidada" value="{{ \Carbon\Carbon::parse($transacao->liquidada)->format("Y-m-d")}}" class="shadow appearance-none border rounded w-full py-2 px-3 my-1 text-gray-700 leading-tight focus:outline-none focus:shadow-outline "/>
            </div>
